...
import { collection, onSnapshot, orderBy, query, doc, getDocs, limit} from 'firebase/firestore';
...
const HomePage = () => {
    ...
    const fetchFeedPosts = () => {
        ...
        // This query gets all posts and orders them by the 'createdAt' field
        ...
        const unsubscribe = onSnapshot(postsQuery, (querySnapshot) => {
          ...
          console.log("Posts fetched for feed:", postsData);
        }, (error) => {
    ...
    const handleMapPress = () => {
        router.push('/subPages2/MapScreen');
    };
...
//     useAnimatedScrollHandler,
// } from 'react-native-reanimated';
// import { GestureDetector, PanGestureHandler, Gesture } from 'react-native-gesture-handler';

// import { collection, onSnapshot, orderBy, query, doc, getDocs, limit, where } from 'firebase/firestore';
// import { signOut, onAuthStateChanged } from 'firebase/auth';
// import { auth, db } from '../../firebaseConfig';
...
//                     // Listen to Trip Diaries inside the posts listener's callback
...
//                     );
//                     unsubscribeTrips = onSnapshot(tripsQuery, async (tripQuerySnapshot) => {
//                         const tripsData = [];
//                         for (const docSnapshot of tripQuerySnapshot.docs) {
//                             const trip = { id: docSnapshot.id, type: 'trip', ...docSnapshot.data() };
//                             tripsData.push(trip);
//                         }

//                         // Combine and sort all data
//                         let combinedFeed = [...postsData, ...tripsData];
//                         combinedFeed.sort((a, b) => {
//                             const dateA = (a.type === 'post' ? a.createdAt?.toDate() : a.publishedAt?.toDate()) || new Date(0);
//                             const dateB = (b.type === 'post' ? b.createdAt?.toDate() : b.publishedAt?.toDate()) || new Date(0);
//                             return dateB.getTime() - dateA.getTime();
//                         });

//                         // Insert discover users section
//                         if (discoverUsers.length > 0) {
//                             const insertIndex = Math.min(Math.floor(Math.random() * 5) + 3, combinedFeed.length);
//                             combinedFeed.splice(insertIndex, 0, { id: 'discover-section', type: 'discover', users: discoverUsers });
//                         }
//                         setFeedData(combinedFeed);
//                         setLoading(false);
//                     }, (error) => {
//                         console.error("Error listening to trip diaries:", error);
//                         Alert.alert("Error", "Failed to load trip diaries.");
//                         setLoading(false);
//                     });
//                 }, (error) => {
//                     console.error("Error listening to posts:", error);
